...
def save_audio(noisy_signal, separated_signals, target, save_path, samplerate):
    ...
    write(f"{save_path}/clean_1.wav", 16000, noisy_signal.astype(np.float32))
...
si_sdrs = []
save_path = "/dsi/gannot-lab/datasets/mordehay/Result/test_for_git/Sepformer_Whamr_tt8kmax_anechoic_target"

for i, path in tqdm(enumerate(pathes_file)):
    with open(path, "rb") as f:
            _, noisy_signal, _, speakers_target, _ = pickle.load(f)
    name = PurePosixPath(path).name
    est_sources = model.separate_file(path=path, batch=noisy_signal, fs_file=16000) #shape = [1, T, 2]
    est_sources = torch.permute(est_sources, (0, 2, 1))
    #est_sources = torch
    #os.remove("/home/dsi/moradim/OurBaselineModels/" + name)
    
    
    speakers_target = torch.tensor(speakers_target, device="cuda")
    ...
    if i < 30:
        ...
        save_audio(noisy_signal ,est_sources, speakers_target, save_path_s, samplerate=8000)
