...
class UConvBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        for k in range(1, self.depth):
            ...
            output.append(out_k)
            #print(f"the {k} out is {out_k.shape}")

        # Gather them now in reverse order
        #print(output.pop(-1).shape)
        for _ in range(self.depth-1):
            resampled_out_k = self.upsampler(output.pop(-1))
            #print(resampled_out_k.shape)
...
class SuDORMRF(nn.Module):
    def __init__(self,
        ...
        self.lcm = abs(self.enc_kernel_size // 2 * 2 **
                       ...
                       2 ** self.upsampling_depth) #320 it's for the 16 sampling rate 
        """abs(self.enc_kernel_size // 2 * 2 **
                       self.upsampling_depth) // math.gcd(
                       self.enc_kernel_size // 2,
