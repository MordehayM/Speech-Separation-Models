...
save_path = '/dsi/gannot-lab/datasets/mordehay/Result/test_for_git/Test_Sudo_data_partial_overlap_wham_16k_whamr/'
...
if GPU:
    ...
    path_data = "/dsi/gannot-lab/datasets/whamr/wav16k/max/tt/mix_both_reverb/*.wav"
    pathes_file = glob.glob(path_data)
    si_sdrs = []
    ...
    if resample_bool:
        ...
        for i, path in tqdm(enumerate(pathes_file)):
            ...
            s1_path = "/dsi/gannot-lab/datasets/whamr/wav16k/max/tt/mix_both_reverb/" + name
            s2_path = "/dsi/gannot-lab/datasets/whamr/wav16k/max/tt/mix_both_reverb/" + name
            _, s1 = read(s1_path)
            _, s2 = read(s2_path)
            s1 = torch.tensor(s1, device="cuda:0")
            s2 = torch.tensor(s2, device="cuda:0")
