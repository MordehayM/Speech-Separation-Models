...
def save_audio(noisy_signal, separated_signals, target, save_path):
    ...
    write(f"{save_path}/clean_0.wav", 8000, target_audio1.astype(np.float32))
    write(f"{save_path}/clean_1.wav", 8000, target_audio2.astype(np.float32))
    write(f"{save_path}/mix.wav", 16000, noisy_signal.astype(np.float32))

#df = pd.read_csv("/dsi/gannot-lab/datasets/mordehay/data_wham_partial_overlap/test/csv_files/with_wham_noise_res.csv")
#pathes_file = df["path_file"]
...
resample = torchaudio.transforms.Resample(orig_freq=16000, new_freq=8000).to("cuda")
path_data = "/dsi/gannot-lab/datasets/mordehay/data_wham_partial_wsj0_sir0_fixed/test/with_wham_noise_audio//*.p" #"/dsi/gannot-lab/datasets/whamr/wav8k/max/tt/mix_both_reverb/*.wav"
pathes_file = glob.glob(path_data)
si_sdrs = []
save_path = "/dsi/gannot-lab/datasets/mordehay/Result/test_for_git/Sepformer_wham_partial_wsj0_sir0_fixed_data_anechoic_target"
Path(save_path).mkdir(parents=True, exist_ok=True)
for i, path in tqdm(enumerate(pathes_file)):
    with open(path, "rb") as f:
            _, noisy_signal, speakers_target, _, _ = pickle.load(f)
            path_temp = save_path + "/mix.wav"
            write(path_temp, 16000, noisy_signal.astype(np.float32))
    print(speakers_target.shape)
    est_sources = model.separate_file(path=path_temp) #shape = [1, T, 2]
    ...
    speakers_target = resample(speakers_target)
    #print(speakers_target.shape)
