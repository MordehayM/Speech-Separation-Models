    ...
    with torch.no_grad():
        for batch_idx, sample_separation in enumerate(tqdm(data_loader)):
           ...
           # out_separation = model(data)
            ...
            #print(si_sdri)
            save_test_path_full = f"{save_test_path}Batch_{batch_idx}_SiSDRI_{si_sdri.item():.2f}_SiSDR_{torch.mean(si_sdr_spk):.2f}_Reverb_{reverb.item():.2f}_Snr_{snr.item():.2f}_Stoi_{stoi_value.item():.2f}/"
            """########
