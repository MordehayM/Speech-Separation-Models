...
def save_audio(mix_waves, separated_signals, batch_indx, save_path, samplerate, bit16):
    ...
    Path(f"{save_path}Batch_{batch_indx}/").mkdir(parents=True, exist_ok=True)
    if bit16:
        write(f"{save_path}Batch_{batch_indx}/Mixed_0.wav", samplerate, mix_waves.astype(np.float16))
        write(f"{save_path}Batch_{batch_indx}/Speaker_0.wav", samplerate, separated_audio1.astype(np.float16))
        write(f"{save_path}Batch_{batch_indx}/Speaker_1.wav", samplerate, separated_audio2.astype(np.float16))
    else:
        write(f"{save_path}Batch_{batch_indx}/Mixed_0.wav", samplerate, mix_waves.astype(np.float32))
        write(f"{save_path}Batch_{batch_indx}/Speaker_0.wav", samplerate, separated_audio1.astype(np.float32))
        write(f"{save_path}Batch_{batch_indx}/Speaker_1.wav", samplerate, separated_audio2.astype(np.float32))
...
def main(config):
    ...
    checkpoint = torch.load(config.resume, map_location=torch.device('cuda'))
    ...
    if m4a_bool:    
        path_audio = "/home/dsi/moradim/Audio-Visual-separation-using-RTF/movearr_13_10_mm.wav"
    ...
    if robot_sample:    
        path_audio = "/home/dsi/moradim/Audio-Visual-separation-using-RTF/robot_recordings/3_100overlap_snr10_sir0_ff/rec/3_mix.wav"
        #name_run = os.path.basename(path_audio)[:-4]
        wav_name= path_audio[:-4]
    ...
    print(audio.max())
    print(audio.min())
    print(f"The DC before normlizing is: {np.mean(audio)}")
    ...
    print(f"The after normlizing DC is: {np.mean(normalized_audio)}")
    ...
    with torch.no_grad():
        out_separation, output_vad, _, _, _, _ = model(normalized_audio, doa)
    
        
    masks = model.mask_per_speaker
    
    plot_spectrogram(masks, 'Mask in fft domain', save_test_path, i)
    save_audio(normalized_audio, out_separation, i, save_test_path, 16000, bit16)
...
if __name__ == '__main__':
    ...
    args.add_argument('-r', '--resume', default="/dsi/scratch/from_netapp/users/mordehay/Results/Separation_data_wham_partial_overlap_ModoluDial_inputActivity_AttentionBeforeSum_ResidualLN_resume/models/AV_model/0115_172306/model_best.pth",
